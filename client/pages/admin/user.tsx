...
export default function AdminUserPage() {
  ...
  // Apply filters when localUsers or filter values change
  useEffect(() => {
    const usersToFilter = Array.isArray(localUsers) ? localUsers : [];
    ...
    setCurrentPage(1);
  }, [localUsers, searchQuery, selectedStatuses, selectedDate]);
  ...
  const handleBanUser = async (userId: string) => {
    try {
      ...
      if (response.ok) {
        setLocalUsers(prev =>
          prev.map(u => (u.id === userId || u._id === userId ? { ...u, status: 'banned' } : u))
        );
      } else {
  ...
  const handleActivateUser = async (userId: string) => {
    try {
      const response = await fetch(`/api/admin/user/${userId}/activate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
      ...
      if (response.ok) {
        setLocalUsers(prev =>
          prev.map(u => (u.id === userId || u._id === userId ? { ...u, status: 'active' } : u))
        );
      } else {
        const error = await response.json();
        ...
        <div className={styles.contentWrapper}>
          <div className={styles.mainContent}>
            <div className={styles.headerSection}>
              <h1 className={styles.mainTitle}>User Overview</h1>
            </div>
            ...
            <div className={styles.tableContainer} data-tg-tour="📋 User Overview - View all registered users, their roles, and account status. You can manage user accounts from here.">
                        ...
                        <td className={styles.userCell}>
                          <div className={styles.userInfo}>
                            ...
                            <div>
                              ...
                              <div className={styles.userId}>User ID: {u._id || u.id}</div>
                            </div>
                          </div>
                        </td>
                        <td className={styles.emailCell}>{u.email}</td>
                        <td>
                          <span className={u.status === 'active' ? styles.statusActive : styles.statusBanned}>
                            {u.status === 'active' ? 'Active' : 'Ban'}
                          </span>
                        </td>
                        ...
                        <td className={styles.actionCell}>
                          {u.status === 'active' ? (
                            ...
                            <button
                              ...
                              onClick={() => handleActivateUser(u.id || u._id || '')}
                              title="Activate User"
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor" viewBox="0 0 16 16">
                                <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
